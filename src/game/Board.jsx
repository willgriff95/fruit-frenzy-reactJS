  ...
  constructor(props) {
    ...
    evaluateGrid(){        
      this.state.grid && this.state.grid.forEach((row, y) => {
        const draggedPositionArrayCheck = this.state.grid[this.state.draggedYPosition]
        const droppedPositionArrayCheck = this.state.grid[this.state.droppedYPosition]
        if( draggedPositionArrayCheck === row || droppedPositionArrayCheck === row ) {
          const rowMatches = this.evaluateRowColumn(row, y);
      ...
      let columns = []
      for (let i = 0; i < this.state.grid.length + 1; i++) {
          columns.push(this.state.grid && this.state.grid.map(row => row[i]))
      }

      columns.forEach((column, x) => {
        if(columns[this.state.droppedXPosition] === column || columns[this.state.draggedXPosition] === column){
          const columnMatches = this.evaluateRowColumn(column, x)
          columnMatches && columnMatches.forEach(match => {
    ...
    evaluateRowColumn(row, axis){
      ...
      row.forEach((value, index) => {
        ...
        currentValue = value
        if ((previousValue === currentValue) && (currentValue === this.state.draggedValue ||  currentValue === this.state.droppedValue) ) {
          consecutiveCount ++
        } else {
          if (consecutiveCount >= 3) {
            matches.push({ matchStartIndex: index - consecutiveCount, length: consecutiveCount})
